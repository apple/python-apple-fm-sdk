...
# =============================================================================
# 2. Integration regression test: real model + image attachments
# =============================================================================


async def _respond_or_skip_on_transient_error(session, prompt):
    """Send a structured-generation request, skipping the test on system-level
    conditions (e.g. transient memory pressure) that are unrelated to the
    composed_prompt leak this test targets."""
    try:
        return await session.respond(prompt, json_schema=DUMMY_SCHEMA)
    except fm.ImagePromptError as e:
        pytest.skip(f"Image prompts not supported: {e}")
    except fm.GenerationError as e:
        if "system state" in str(e) or "MemoryPressure" in str(e):
            pytest.skip(f"Model unavailable due to transient system state: {e}")
        raise


@pytest.mark.asyncio
async def test_sequential_schema_requests_with_images_do_not_leak_fds(
    session, test_image_path
):
    """Sequential structured-generation requests with image attachments must not
    leak open file descriptors (the end-to-end symptom reported for this bug)."""

    def open_fd_count() -> int:
        return len(os.listdir("/dev/fd"))

    # Warm up so one-time setup costs don't skew the baseline measurement.
    warm_image = fm.ImageAttachment(path=test_image_path)
    await _respond_or_skip_on_transient_error(session, ["Echo hello warmup", warm_image])
    ...
    for i in range(num_requests):
        image = fm.ImageAttachment(path=test_image_path)
        await _respond_or_skip_on_transient_error(session, [f"Echo hello {i}", image])
        del image
        gc.collect()

    final = open_fd_count()
    growth = final - baseline
    assert growth < num_requests, (
        f"open file descriptor count grew by {growth} over {num_requests} "
        f"sequential requests (baseline={baseline}, final={final}) - "
        f"composed_prompt/attachment leak regression"
    )
